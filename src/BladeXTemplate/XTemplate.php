...
use Yng\View\BladeXTemplate\FileSystem;
...
class XTemplate
{
    ...
    protected $config = [
        ...
        'tpl_raw_begin'      => '{!!', // 模板引擎原始数据标签开始标记
        'tpl_raw_end'        => '!!}', // 模板引擎原始数据标签结束标记
    ...
    public function parse(string &$content): void
    {
        ...
        // 还原被替换的rawData标签
        $this->parseRawData($content, true);
    }
        ...
        $func = function ($template) use (&$func, &$regex, &$array, &$extend, &$blocks, &$baseBlocks) {
            if (preg_match($regex, $template, $matches)) {
                if (!isset($array[$matches['name']])) {
                    ...
                    // 取得block标签内容
                    $blocks = array_merge($blocks, $this->parseSection($template));

                    return;
                }
            } else {
                // 取得顶层模板block标签内容
                $baseBlocks = $this->parseSection($template, true);
     ...
     * @return void
     */
    private function parseRawData(string &$content, bool $restore = false): void
    {
        $regex = $this->getRegex($restore ? 'restoreRawData' : 'rawData');
        ...
        if (preg_match_all($regex, $content, $matches, PREG_SET_ORDER)) {
            if (!$restore) {
                ...
                // 替换rawData标签
                foreach ($matches as $match) {
                    $this->rawData[] = substr($match[0], strlen($match[1]), -strlen($match[2]));
                    $content         = str_replace($match[0], "<!--###rawData{$count}###-->", $content);
                    $count++;
                }
            } else {
                // 还原rawData标签
                foreach ($matches as $match) {
                    $content = str_replace($match[0], $this->rawData[$match[1]], $content);
                }
     ...
     * @return array
     */
    private function parseSection(string &$content, bool $sort = false): array
    {
        $regex  = $this->getRegex('section');
        $result = [];
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false): void
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\Yng\\View\\BladeXTemplate\\Taglib\\' . ucwords($tagLib);
        }
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if (isset($this->extend[$first])) {
                            ...
                            // 所有以Yng.打头的以特殊变量对待 无需模板赋值就可以输出
                            $parseStr = $this->parseYngVar($vars);
                        } else {
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $Yng. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parseYngVar(array $vars): string
    {
        $type  = strtoupper(trim(array_shift($vars)));
        $param = implode('.', $vars);
        ...
        switch ($type) {
            ...
            case 'NOW':
                $parseStr = "date('Y-m-d Y:i:s',time())";
            break;
    ...
    private function getRegex(string $tagName): string
    {
        ...
        if ('tag' == $tagName) {
            ...
            switch ($tagName) {
                ...
                case 'rawData':
                    $begin = $this->config['tpl_raw_begin'];
                    $end   = $this->config['tpl_raw_end'];
                    /*
                    ...
                    */
                    ...
                    $pattern_str = '(?<!\\w)(%s)(\\s*(.+?)\\s*)(%s)(?!\\w)';
                    $regex = sprintf($pattern_str,preg_quote($begin),preg_quote($end));
                ...
                case 'extend':
                    ...
                    //     $name = 'name';
                    // }
                    // $regex = $begin . $tagName . "\(\'[\s\S]*\'\)[\s\S]*". $end . $tagName;
                    $regex = $begin . $tagName . "\(\'[\s\S]*\'\)[\s\S]*";
                break;
            }
        }
