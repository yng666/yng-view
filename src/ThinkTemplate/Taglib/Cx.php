...
class Cx extends Taglib
{
    ...
    public function tagVolist(array $tag, string $content): string
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \Yng\Collection || ' . $name . ' instanceof \Yng\Paginator): $' . $key . ' = 0;';
    ...
    public function tagEmpty(array $tag, string $content): string
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || ((' . $name . ' instanceof \Yng\Collection || ' . $name . ' instanceof \Yng\Paginator ) && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
